...
export const getRecommendation = (keyword) => {
  return (dispatch) => {
    axios.get(`http://www.omdbapi.com?apikey=faf7e5bb&s=${keyword}`)
    .then(({data}) => {
        if (data.Search) {
...
export const addMovies = (keyword, page) => {
  return (dispatch) => {
    axios.get(`http://www.omdbapi.com?apikey=faf7e5bb&s=${keyword}&type=movie&page=${page}`)
    .then(({data}) => {
      console.log(data.Search, "ACTION");
      if (data.Search) {
...
export const addSeries = (keyword, page) => {
  return (dispatch) => {
    axios.get(`http://www.omdbapi.com?apikey=faf7e5bb&s=${keyword}&type=series&page=${page}`)
    .then(({data}) => {
      if (data.Search) {
...
export const addEpisodes = (keyword, page) => {
  return (dispatch) => {
    axios.get(`http://www.omdbapi.com?apikey=faf7e5bb&s=${keyword}&type=episode&page=${page}`)
    .then(({data}) => {
      if (data.Search) {
